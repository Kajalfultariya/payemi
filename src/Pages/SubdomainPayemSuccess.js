...
const SubdomainPayemSuccess = (props) => {
  ...
  // guest localstorage
  const guest_amount = localStorage.getItem("amount");
  ...
  const id = fetchBill?.map((data) => data.profile_id);
  console.log(id);
  //  biller id
  const biller_id = fetchBill?.map((data) => data.id);
  ...
  const fetchData = async () => {
    const res = await getBillDetailsNoAuthData(biller_id);
    console.log(res);
    if (res?.error) {
      setError(res?.message);
    } else {
      setBillerInfo(res?.data);
    }
  };
  ...
  const handleshowToggle = async () => {
    ...
    const url = `https://api.payemi.net/getcashback-noauth/?bill_id=${biller_id}&profile_id=${id}`;

    const res = await getCashBackNoAuthData(biller_id, id);
    if (res?.error) {
      setError(res?.message);
    } else {
      setCashback(res);
  ...
  const handleRedem = async () => {
    const res = await getScratchCardNoAuthData(id, biller_id);
    ...
    if (res?.error) {
      ...
      window.location.href = window.location.href
        .replace(subDomain + ".", "")
        .replace("success", `verify-otp/${mobileNumber}`);
    }
  };
          ...
          <div className="py-0 px-3">
            <div className="container-fluid">
              <div className="mt-4">
                ...
                <div className="row mb-3">
                  <div className="col-12">
                    <div className="d-flex align-center justify-between">
                      ...
                      <p className="m-0">
                        <strong>{guest_biller_info?.billerName}</strong>
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="row mb-3">
                    <div className="col-12">
                      <div className="d-flex align-center justify-between">
                        ...
                        <p className="m-0">
                          <strong>{data.RespDueDate}</strong>
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="row mb-3">
                    <div className="col-12">
                      <div className="d-flex align-center justify-between">
                        ...
                        <p className="m-0">
                          <strong>{data.RespBillNumber}</strong>
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {data["biller_additional_info"]["Total Outstanding Amount"] && (
                  <div className="row mb-3">
                    <div className="col-12">
                      <div className="d-flex align-center justify-between">
                  ...
                  <div className="row mb-3">
                    <div className="col-12">
                      <div className="d-flex align-center justify-between">
                        ...
                        <p className="m-0">
                          <strong>{data["transation_status"]}</strong>
                        </p>
                      </div>
                    </div>
                  </div>
...
export default SubdomainPayemSuccess;
